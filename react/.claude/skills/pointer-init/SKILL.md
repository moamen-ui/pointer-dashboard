...
The loader loads `http://localhost:8090/pointer.js`, then appends a `<pointer-feedback>` element.
...
widget only when `VITE_POINTER_SERVER` is a URL, so a build that must ship without Pointer
simply leaves it empty and carries zero Pointer code paths.
...
<pointer-feedback
  project="<project-key>"
  server="http://localhost:8090"></pointer-feedback>
```
...
<script>
  (function () {
    function mount() {
      ...
      el.setAttribute("server", "http://localhost:8090");
      document.body.appendChild(el);
    }
...
- **Privacy & self-hosting:** For the full engineering breakdown of what the widget captures, what is never captured, retention and deletion semantics, and self-hosting boundaries, see `http://localhost:8090/data.html`.
