...
> **For AI agents:** This document explains how this Angular app generates
...
The .NET API (the [`poitner-api`](https://github.com/moamen-ui/poitner-api) repo) must be running
on `http://localhost:8090`. From a clone of that repo:
...
Every controller action that the dashboard uses must declare its response type:
...
```bash
# Ensure API is running, then from this repo's root:
npm run generate-services
```
...
- `openapi.json` — downloaded spec, not hand-written

## Config Files Reference

| File | Purpose |
|---|---|
| `orval.config.ts` | Orval generation config (client mode, tags, output paths) |
| `scripts/generate-services.mjs` | Download + generate script |
| `package.json` | `generate-services` npm script |
| `tsconfig.json` | `@api/*` path alias |
| `src/app/core/auth/auth.interceptor.ts` | Unified API interceptor (envelope unwrap) |
| `src/app/core/api/extract-message.ts` | Error message extraction utility |
| `src/environments/environment.ts` | `apiBase` URL (dev → `localhost:8090`) |
| `src/environments/environment.prod.ts` | `apiBase` for prod builds (swapped in via `angular.json` `fileReplacements`) |
