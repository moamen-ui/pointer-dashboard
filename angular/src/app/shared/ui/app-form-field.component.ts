...
/**
 * Field chrome (label + projected control + hint/error), matching React's FormField.tsx and
 * Vue's FormField.vue exactly: `gap-2` (8px) on the flex column is the ONE place that sets the
 * label-to-control gap and the control-to-hint/error gap in this app. Change it here (and mirror
 * the same value in the other two apps' FormField) to move the gap everywhere a field uses this
 * wrapper — never add a per-field `mb-*`/`mt-*` override in a feature component.
 */
@Component({
