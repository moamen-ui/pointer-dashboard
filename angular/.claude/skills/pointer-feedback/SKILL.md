...
---
<!-- pointer-skill-version: 0.0.0-dev+584fd0e1ecda1506d10235933785fa1c458792c4 -->
...
**Pointer API**, partitioned by project and tied to the author's real account. This skill works
...
- **"What are the Pointer comments?"** → `npx pointer-feedback list` and format the result.
- **"Apply the pending Pointer comments"** → `npx pointer-feedback apply`, edit, stage, `--mark`.
...
If you are running in an MCP-capable environment (Claude Code, Cursor, Windsurf, OpenCode), you can connect to Pointer's stdio MCP server instead of shelling out:
```json
...
4. **Stage and mark — the CLI commits.** `git add -- <only the files this item touched>`, then run the
   ...
   - **One commit** (`commitStyle` = 1, default): stage every item first, then once →
     `npx pointer-feedback apply --mark all --reply "Applied N Pointer comments — <summary>"`.
...
shim served from `http://localhost:8090/pointer.sh` and refreshed by `npx pointer-feedback update`. It reads
...
- This file was installed by fetching `http://localhost:8090/skill.md` into
